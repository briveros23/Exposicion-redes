...
La **regresión kernel** se aplica cuando la variable respuesta $y$ y la explicativa $x$ son continuas. Se observan $n$ pares $(x_i, y_i)$ siguiendo el modelo no paramétrico:

$$
y_i = m(x_i) + \epsilon_i
$$

-   $\epsilon_i$ son variables independientes.
-   $m(x)$ es una función regresora sin forma específica.

La función kernel $K$ que define $m(x)$ debe ser:

-   Simétrica, no negativa y continua.
-   Decreciente en $[0, \infty)$ y tendiendo a 0 cuando el argumento es infinito.


<center>![](Kernels.svg.png)</center>
